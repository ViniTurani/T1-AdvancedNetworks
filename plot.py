...
#             "Tempo (s)",
#             "Throughput RTP (kB/s)",
#             "RTP Throughput (ifstat)",
#             f"{out_dir}/rtp_throughput.png",
#         )
...
#             "Tempo (s)",
#             "Throughput UDP (bps)",
#             f"Iperf {idx} Throughput",
#             f"{out_dir}/iperf_{idx}_thr.png",
#         )
#         plot_line(
#             df_ip["end"],
#             df_ip["jitter"],
...
#             df_plot["delta"],
#             "Tempo (s)",
#             "Atraso entre Pacotes (s)",
#             "RTP Packet Inter-arrival Delay",
#             f"{out_dir}/rtp_interarrival.png",
#         )
...
IFSTAT_FILE = "h2-eth0-ifstat.txt"
SAMPLE_INTERVAL = 0.5  # segundos entre amostras
...
def parse_ifstat(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            # só linhas com exatamente 2 números flutuantes
            if len(parts) == 2 and all(re.match(r"^\d+(\.\d+)?$", p) for p in parts):
                in_kb.append(float(parts[0]))
                out_kb.append(float(parts[1]))
                times.append(sample * SAMPLE_INTERVAL)
                sample += 1
    return pd.DataFrame({"time": times, "in_kB": in_kb, "out_kB": out_kb})


def main():
    df = parse_ifstat(IFSTAT_FILE)
    # throughput RTP em Mbit/s (saída do switch para h2)
    df["rtp_Mbps"] = df["in_kB"] * 8.0 / 1000.0

    plt.figure()
    plt.plot(df["time"], df["rtp_Mbps"])
    plt.xlabel("Tempo (s)")
    plt.ylabel("RTP Throughput (Mbit/s)")
    plt.title("RTP Throughput ao Longo do Tempo")
    plt.tight_layout()
    plt.savefig("rtp_throughput.png")
    plt.show()
